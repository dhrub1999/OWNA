...
import type { SocialPlatform } from "@/lib/blocks/definitions";

/**
 * Brand glyphs for the social block.
 *
 * The path data below is inlined from Simple Icons (https://simpleicons.org),
 * which publishes it under CC0-1.0. It is committed rather than imported so the
 * public profile ships nothing but the handful of paths it actually renders —
 * pulling a 3,400-icon package into the bundle for eleven glyphs would work
 * against the whole point of a fast public page.
 *
 * Regenerate by re-extracting the `d` attribute from
 * node_modules/simple-icons/icons/<slug>.svg.
 *
 * LinkedIn is not in Simple Icons (it was removed at the trademark holder's
 * request), so its glyph is reconstructed here from the mark's own geometry.
 *
 * Every icon is decorative: the accessible name lives on the surrounding link,
 * so these carry aria-hidden and no <title>.
 */
...
export function InstagramIcon(props: IconProps) {
  return <Brand d={INSTAGRAM_PATH} {...props} />;
}

export function XIcon(props: IconProps) {
  return <Brand d={X_PATH} {...props} />;
}

export function GithubIcon(props: IconProps) {
  return <Brand d={GITHUB_PATH} {...props} />;
}

export function YoutubeIcon(props: IconProps) {
  return <Brand d={YOUTUBE_PATH} {...props} />;
}

export function DiscordIcon(props: IconProps) {
  return <Brand d={DISCORD_PATH} {...props} />;
}

export function TiktokIcon(props: IconProps) {
  return <Brand d={TIKTOK_PATH} {...props} />;
}

export function FacebookIcon(props: IconProps) {
  return <Brand d={FACEBOOK_PATH} {...props} />;
}

export function TwitchIcon(props: IconProps) {
  return <Brand d={TWITCH_PATH} {...props} />;
}

export function DribbbleIcon(props: IconProps) {
  return <Brand d={DRIBBBLE_PATH} {...props} />;
}

export function BehanceIcon(props: IconProps) {
  return <Brand d={BEHANCE_PATH} {...props} />;
}

/**
 * LinkedIn's mark reconstructed from its own geometry: the dot and stem of the
 * "i", then the "n". Kept in the same 24x24 box as the Simple Icons glyphs so
 * it optically matches them at every size.
 */
...
/** Platform -> component. `website` and `email` fall back to Lucide. */
